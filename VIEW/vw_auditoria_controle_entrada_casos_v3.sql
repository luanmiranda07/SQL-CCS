...
COALESCE(dde2.data_dde, p.entrada) AS 'DATA FINAL (REGRA DDE/ENTRADA)',
p.entrada AS "DATA CADASTRO",
dde2.data_dde AS "DDE"
...
LEFT JOIN  tab_acao ta ON ta.sigla = p.acao
...
LEFT JOIN (
    SELECT
        id_processo, 
        MIN(data_hora_lan) AS data_dde
    FROM tramitacao
    WHERE evento = 'DDE'
    GROUP BY id_processo
) dde2 ON dde2.id_processo = p.pj


where p.incidente = 0
and p.id_unidade = 1
and p.acao not in ('CUMSEND','CUMPPRS','INST','CSDT','CSPT','CP','AP','EMBE','SEG','ARESP')
